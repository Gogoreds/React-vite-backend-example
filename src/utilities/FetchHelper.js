...
export default class FetchHelper {
  ...
  async save() {
    ...
    let toSave = { ...this };
    delete toSave.restEntity;
    let result = await (await fetch(`/api/${this.restEntity}${method === 'PUT' ? '/' + this.id : ''}`, {
      method,
  ...
  async delete() {
    ...
    return await (await fetch(`/api/${this.restEntity}/${this.id}`, {
      method: 'DELETE'
    }));
  }
  ...
  get(object, property) {
    let func = new Function('baseClass', `
      return class ${property} extends baseClass {
        ...
        restEntity = '${property.toLowerCase() + 's'}';
      }
    `);
